...
from src.module_enterprise.dbmodel.models import (
    ComplaintFeedback,
    EmployeeDailyReport,
    IntentCustomer,
    StudentScore,
)
from src.module_enterprise.pdcmodel.schemas import (
    APIResponse,
...
from src.module_enterprise.dao.customer_dao import customer_dao
from src.module_enterprise.dao.report_dao import report_dao
from src.module_enterprise.dao.complaint_dao import complaint_dao
from src.module_enterprise.dao.score_dao import score_dao
...
async def _get_or_404(crud, id: int, db: AsyncSession, response_schema: type):
...
async def _create_record(crud, data, db: AsyncSession, response_schema: type):
    """创建一条新记录并返回。
...
async def _update_record(crud, id: int, data, db: AsyncSession, response_schema: type):
...
async def _delete_record(crud, id: int, db: AsyncSession):
...
async def list_customers(
    ...
    result = await customer_dao.list(db, page=page, page_size=page_size, **kwargs)
...
async def get_customer(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """获取单条意向客户记录。"""
    return await _get_or_404(customer_dao, id, db, IntentCustomerResponse)
...
async def create_customer(data: IntentCustomerCreate, db: AsyncSession = Depends(get_enterprise_db)):
    """创建新的意向客户记录。"""
    return await _create_record(customer_dao, data, db, IntentCustomerResponse)
...
async def update_customer(id: int, data: IntentCustomerUpdate, db: AsyncSession = Depends(get_enterprise_db)):
    """更新意向客户记录。"""
    return await _update_record(customer_dao, id, data, db, IntentCustomerResponse)
...
async def delete_customer(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """删除意向客户记录。"""
    return await _delete_record(customer_dao, id, db)
...
async def list_reports(
    ...
    result = await report_dao.list(db, page=page, page_size=page_size, **kwargs)
...
async def get_report(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """获取单条员工日报记录。"""
    return await _get_or_404(report_dao, id, db, EmployeeDailyReportResponse)
...
async def create_report(data: EmployeeDailyReportCreate, db: AsyncSession = Depends(get_enterprise_db)):
    """创建新的员工日报记录。"""
    return await _create_record(report_dao, data, db, EmployeeDailyReportResponse)
...
async def update_report(id: int, data: EmployeeDailyReportUpdate, db: AsyncSession = Depends(get_enterprise_db)):
    """更新员工日报记录。"""
    return await _update_record(report_dao, id, data, db, EmployeeDailyReportResponse)
...
async def delete_report(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """删除员工日报记录。"""
    return await _delete_record(report_dao, id, db)
...
async def list_complaints(
    ...
    result = await complaint_dao.list(db, page=page, page_size=page_size, **kwargs)
...
async def get_complaint(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """获取单条投诉反馈记录。"""
    return await _get_or_404(complaint_dao, id, db, ComplaintFeedbackResponse)
...
async def create_complaint(data: ComplaintFeedbackCreate, db: AsyncSession = Depends(get_enterprise_db)):
    """创建新的投诉反馈记录。"""
    return await _create_record(complaint_dao, data, db, ComplaintFeedbackResponse)
...
async def update_complaint(id: int, data: ComplaintFeedbackUpdate, db: AsyncSession = Depends(get_enterprise_db)):
    """更新投诉反馈记录。"""
    return await _update_record(complaint_dao, id, data, db, ComplaintFeedbackResponse)
...
async def delete_complaint(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """删除投诉反馈记录。"""
    return await _delete_record(complaint_dao, id, db)
...
async def list_scores(
    ...
    result = await score_dao.list(db, page=page, page_size=page_size, **kwargs)
...
async def get_score(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """获取单条学生成绩记录。"""
    return await _get_or_404(score_dao, id, db, StudentScoreResponse)
...
async def create_score(data: StudentScoreCreate, db: AsyncSession = Depends(get_enterprise_db)):
    """创建新的学生成绩记录。"""
    return await _create_record(score_dao, data, db, StudentScoreResponse)
...
async def update_score(id: int, data: StudentScoreUpdate, db: AsyncSession = Depends(get_enterprise_db)):
    """更新学生成绩记录。"""
    return await _update_record(score_dao, id, data, db, StudentScoreResponse)
...
async def delete_score(id: int, db: AsyncSession = Depends(get_enterprise_db)):
    """删除学生成绩记录。"""
    return await _delete_record(score_dao, id, db)
